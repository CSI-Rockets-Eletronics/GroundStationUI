...
#define FC_MIN(a,b) ((int) (a) < (int) (b)? (a) : (b))
#define FC_MAX(a,b) ((int) (a) > (int) (b)? (a) : (b))
...
static char* stringtochar(std::string str) {
    char output[str.size() + 1];
    std::strcpy(output, str.c_str());
    char* ret = strdup(output);
    return ret;
}
...
#pragma GCC diagnostic ignored "-Wunused-function"
static FC_Rect FC_RenderCenter(FC_Font* font, FC_Target* dest, float x, float y, FC_Scale scale, std::string text);
#pragma GCC diagnostic ignored "-Wunused-function"
...
static_inline FC_Rect FC_RectUnion(FC_Rect A, FC_Rect B)
{
    ...
    {
        FC_Rect result = {(int) x, (int) y, FC_MAX(0, x2 - x), FC_MAX(0, y2 - y)};
        return result;
    }
}
...
static void FC_Init(FC_Font* font)
{
    ...
    font->filter = FC_FilterEnum::FC_FILTER_NEAREST;
...
Uint8 FC_UploadGlyphCache(FC_Font* font, int cache_level, SDL_Surface* data_surface)
{
	...
	GPU_SetAnchor(new_level, 0.5f, 0.5f);  // Just in case the default is different
	if (FC_GetFilterMode(font) == FC_FilterEnum::FC_FILTER_LINEAR)
...
#else
Uint8 FC_LoadFont(FC_Font* font, FC_Target* renderer, std::string filename_ttf, Uint32 pointSize, SDL_Color color, int style)
#endif
{
    SDL_RWops* rwops;
    ...
    SDL_ClearError();
    rwops = SDL_RWFromFile(filename_ttf.c_str(), "rb");
...
FC_Rect FC_Draw(FC_Font* font, FC_Target* dest, float x, float y, std::string formatted_text, ...) {
    char* formattedText = stringtochar(formatted_text);
    if (formatted_text.empty() || font == NULL) {
...
static FC_StringList* FC_Explode(std::string text, char delimiter) {
    char* textC = stringtochar(text);
    FC_StringList* head;
...
static void FC_RenderAlign(FC_Font* font, FC_Target* dest, float x, float y, int width, FC_Scale scale, FC_AlignEnum align, std::string text) {
    switch(align) {
        case FC_AlignEnum::FC_ALIGN_LEFT:
            // FC_RenderLeft(FC_Font*, SDL_Renderer*, float, float, FC_Scale, std::string)
            FC_RenderLeft(font, dest, x, y, scale, text);
            break;
        case FC_AlignEnum::FC_ALIGN_CENTER:
            FC_RenderCenter(font, dest, x + width/2, y, scale, text);
            break;
        case FC_AlignEnum::FC_ALIGN_RIGHT:
...
FC_Rect FC_DrawBox(FC_Font* font, FC_Target* dest, FC_Rect box, std::string formatted_text, ...) {
    ...
    FC_DrawColumnFromBuffer(font, dest, box, NULL, FC_MakeScale(1,1), FC_AlignEnum::FC_ALIGN_LEFT);
...
FC_Rect FC_DrawBoxScale(FC_Font* font, FC_Target* dest, FC_Rect box, FC_Scale scale, std::string formatted_text, ...) {
    ...
    FC_DrawColumnFromBuffer(font, dest, box, NULL, scale, FC_AlignEnum::FC_ALIGN_LEFT);
...
FC_Rect FC_DrawColumn(FC_Font* font, FC_Target* dest, float x, float y, Uint16 width, std::string formatted_text, ...) {
    ...
    FC_DrawColumnFromBuffer(font, dest, box, &total_height, FC_MakeScale(1,1), FC_AlignEnum::FC_ALIGN_LEFT);
...
FC_Rect FC_DrawColumnScale(FC_Font* font, FC_Target* dest, float x, float y, Uint16 width, FC_Scale scale, std::string formatted_text, ...) {
    ...
    FC_DrawColumnFromBuffer(font, dest, box, &total_height, scale, FC_AlignEnum::FC_ALIGN_LEFT);
...
static FC_Rect FC_RenderRight(FC_Font* font, FC_Target* dest, float x, float y, FC_Scale scale, char* text) {
    FC_Rect result = {(int) x, (int) y, 0, 0};
    if (std::strlen(text) == 0 || font == NULL) {
        return result;
    }
...
FC_FilterEnum FC_GetFilterMode(FC_Font* font) {
    if (font == NULL) {
        return FC_FilterEnum::FC_FILTER_NEAREST;
    }
...
Uint16 FC_GetHeight(FC_Font* font, char* formatted_text, ...) {
    if (std::strlen(formatted_text) == 0 || font == NULL) {
        return 0;
    }
...
FC_Rect FC_GetCharacterOffset(FC_Font* font, Uint16 position_index, int column_width, std::string formatted_text, ...) {
    ...
    if (formatted_text.empty() || column_width == 0 || position_index == 0 || font == NULL) {
        return result;
    }
...
Uint16 FC_GetColumnHeight(FC_Font* font, Uint16 width, std::string formatted_text, ...) {
    ...
    if (formatted_text.empty() || width == 0) {
        return font->height;
    }
...
Uint16 FC_GetPositionFromOffset(FC_Font* font, float x, float y, int column_width, FC_AlignEnum align, std::string formatted_text, ...) {
    ...
    if (formatted_text.empty() || column_width == 0 || font == NULL) {
        return 0;
    }
